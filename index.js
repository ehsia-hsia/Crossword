...
//   input[i].addEventListener("keyup", checkLetter);
// }
...
let input = document.getElementsByTagName("input");
for (let i = 0; i < input.length; i++) {
  input[i].addEventListener("keyup", test);
  function test() {
    for (let j = 0; j < wordKey.length; j++) {
      // iterate through array
      if (input[i] == wordKey[j][0]) {
        // match input to nested array 0, which holds id
        if (input[i].value == wordKey[j][1]) {
